...
class AssignmentProgress(BaseModel):
    ...
    student_id: Mapped[int] = mapped_column(Integer, ForeignKey('our_users.id'), nullable=False)
    assignment_id: Mapped[int] = mapped_column(Integer, ForeignKey('assignments.id'), nullable=False)
    is_completed: Mapped[bool] = mapped_column(Boolean, default=False)
    submission_file_key: Mapped[str] = mapped_column(String, nullable=True)  # S3 file key for submission
