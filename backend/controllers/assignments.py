...
from backend.schemas.assignment import (
    AssignmentCreate, AssignmentWithCommentsResponse, 
...
@router.get("/{assignment_id}/with-progress", response_model=AssignmentWithProgressResponse)
...
async def update_assignment(
    ...
    course = db.query(Course).filter(Course.id == course_id).first()
    if current_user.get('role') != 'admin' and course.teacher_id != current_user.get('user_id'):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
