...
from backend.schemas.progress import (
    AssignmentProgressCreate, AssignmentProgressResponse, AssignmentProgressUpdate,
...
def check_enrollment(db: Session, student_id: int, course_id: int):
...
def update_course_progress(db: Session, student_id: int, course_id: int):
    ...
    if course_progress:
        ...
        completed_count = db.query(AssignmentProgress).join(
            Assignment, Assignment.id == AssignmentProgress.assignment_id
        ).filter(
            Assignment.course_id == course_id,
...
async def create_or_update_assignment_progress(
    ...
    if progress_data.student_id != user_id and current_user.get('role') not in ['teacher', 'admin']:
        raise HTTPException(
    ...
    progress = db.query(AssignmentProgress).filter(
...
@router.get("/assignments/{assignment_id}/student/{student_id}", response_model=AssignmentProgressResponse)
...
async def update_assignment_progress(
    ...
    if progress_data.submission_file_key and progress_data.submission_file_key != progress.submission_file_key:
        progress.submitted_at = datetime.now()
...
@router.get("/courses/{course_id}/student/{student_id}", response_model=CourseProgressResponse)
...
@router.get("/courses/{course_id}/assignments", response_model=List[AssignmentWithProgressResponse])
